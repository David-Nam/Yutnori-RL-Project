...
TIMESTEPS_LABEL="${TIMESTEPS_LABEL:-50m}"
N_ENVS="${N_ENVS:-32}"
VEC_ENV="${VEC_ENV:-subproc}"
DEVICE="${DEVICE:-cuda}"
CHECKPOINT_FREQ="${CHECKPOINT_FREQ:-5000000}"
RUNS_ROOT="${RUNS_ROOT:-runs/ppo_common_rule_50m_backdo_subproc}"
LOGS_ROOT="${LOGS_ROOT:-logs/ppo_common_rule_50m_backdo_subproc}"
...
printf '\n[%s] Full-backdo 50M common-rule PPO training started\n' "$(date -Is)"
...
for seed in "${SEED_ARGS[@]}"; do
  ...
  if [[ -n "${EVAL_SEED_FILE}" ]]; then
    output_path="${run_dir}/eval_common_rule_paired_full_backdo.json"
    eval_seed_args=(--seed-file "${EVAL_SEED_FILE}")
  else
    output_path="${run_dir}/eval_common_rule_paired_full_backdo_$((EVAL_SEED_COUNT * 2)).json"
    eval_seed_args=(
  ...
  printf '\n[%s] Full-backdo paired evaluation: %s\n' "$(date -Is)" "${run_name}"
...
printf '\n[%s] Full-backdo 50M training and evaluation finished\n' "$(date -Is)"
